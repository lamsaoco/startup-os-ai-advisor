...
def transform_and_load(**context):
    ...
    from ingestion.loader import (
        get_connection, init_schema, upsert_document,
        delete_chunks_for_document, insert_chunks,
    )
    ...
    try:
        ...
        conn.close()
        # Clean up bronze file to free disk space after processing
        if os.path.exists(bronze_file):
            os.remove(bronze_file)
            print(f"[Cleanup] Deleted bronze file: {bronze_file}")
